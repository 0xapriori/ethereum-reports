...
- **Ethereum's most durable applications share a design property the last cycle systematically under-valued: they cannot be changed.** WETH9 has settled ~275M operations since the Merge alone (including reads and state-changing calls; ~179M of those were `transfer` and ~24M `transferFrom` per Limit Break's Dune data) without a single line of code changing since its 2017 deployment ([Limit Break, 2024](https://medium.com/limit-break/introducing-wrapped-native-a-modern-replacement-for-wrapped-ether-cc0431c8a964)). Uniswap V2 is still the reference DEX nearly six years after launch. Liquity V1 has run a governance-free stablecoin with no admin key since April 2021. Tornado Cash's immutable contracts outlasted the OFAC sanctions that tried to kill them. These are not accidents; they are a design philosophy the ecosystem drifted away from and is only now returning to.

- **The philosophical frame is credible neutrality.** Vitalik's four properties — *don't write specific people or outcomes into the mechanism; open source and publicly verifiable; keep it simple; don't change it often* — are exactly the properties an immutable deployment realizes by construction. Sanctuary technology, the frame of [state-of-ethereum-march-2026.md](/ethereum/state-of-ethereum-march-2026.md), names the same target from a different angle: infrastructure designed so no one, including the creators, can revoke it. Immutability is what turns a piece of code into infrastructure.

- **At least five structural forces pulled builders away from immutability between 2020 and 2024**, not two. L1 gas costs pushed users onto governed L2s and parallel EVMs. Token-warrant economics made governed protocols easier to finance. Regulatory pressure after Tornado Cash made immutability a personal liability for founders. Institutional demand-side network effects pulled toward protocols with responsible parties. And founder psychology — the status economy of ongoing governance participation — selected against "deploy and walk away." The first two are loosening; the other three are not.

- **The mainnet cost constraint is loosening, though the framing should be probabilistic.** Pectra (May 7, 2025) raised the gas limit to 36M. Fusaka (Dec 3, 2025) enabled a ceiling of 150M via PeerDAS and EIP-7918; actual validator-signaled gas limits have ramped more gradually, into the ~60-100M range by April 2026. The Strawmap targets ~1 gigagas/sec on L1 by the late 2020s ([strawmap.org](https://strawmap.org/)). Base case (~55%): minority adoption of immutability in new protocols; upside (~25%): immutable-on-L1 becomes the default primitive pattern by 2029; downside (~20%): points, airdrops, and RWA-with-governance reintroduce token-warrant economics under new names.

- **Morpho Blue is the canonical modern reference design, with Ajna as the more radical case.** Morpho Blue: 650 lines of Solidity, singleton contract, no upgradability, permissionless market deployment. Deposits reached ~$10B by late 2025 and sit at ~$5.8-6.9B in April 2026 per DefiLlama. Ajna Finance goes further — no oracle, no governance token, no admin surface at all. Together they define the spectrum of immutable lending.

- **Not every application belongs on immutable mainnet.** Yield strategies, oracle feeds, lending risk curation, L2 sequencer software, and consumer-facing UX layers benefit from iteration. The pattern that works is *layered immutability* — subsidiarity, in political-theory terms: the primitive is fixed, the periphery is governed, users choose which periphery to trust. Uniswap V2, Morpho Blue, and ENS (immutable registry, governed registrar/resolvers) converge on this split.

- **The 2022-2025 track record is substantial enough to cite rather than argue about, but requires a survivorship disclaimer.** Tornado Cash sanctions (Aug 2022), appellate reversal (Nov 2024), Treasury de-listing (Mar 2025); Roman Storm's Aug 2025 split verdict; the Euler upgrade patch that introduced its own $197M exploit vector; L2 sequencer outages across Base, Arbitrum, and the Superchain. The winners cited in this report are survivors; the population of immutable deployments from 2018-2022 includes many that are exploited, dead, or dormant, and a complete denominator is not publicly available.

- **Liquity V1 uses Chainlink as primary oracle with Tellor as fallback.** Any claim that Liquity is strictly immutable must acknowledge this dependency. The redundancy is the design; the oracle remains part of the trust surface.

- **The most valuable work on Ethereum for the next several years is likely to be quiet, boring, and permanent.** Not tokens, not governance forums, not another L2. Deploying the right primitive once, correctly, and walking away from the admin key.
