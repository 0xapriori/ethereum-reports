...
- **Cross-chain flash loans are the only genuinely novel product class enabled by synchronous composability.** The CRATE protocol (February 2025, academic) demonstrates feasibility with 0.32-0.75x gas overhead relative to standard ZK rollup transactions. But flash loans are an efficiency tool for sophisticated actors, not a mass-market product.
